...
INFERENCE_URL = f"http://localhost:9001/{LANE_SEGMENTATION_MODEL['MODEL_ID']}/{LANE_SEGMENTATION_MODEL['VERSION']}"

# Create output directory
OUTPUT_DIR = "lane_segmentation_output"
...
def camera_loop():
    ...
    print("Starting OAK-D Pro with Lane Segmentation model from Roboflow...")
    print(f"Model: {LANE_SEGMENTATION_MODEL['MODEL_ID']}/{LANE_SEGMENTATION_MODEL['VERSION']}")
    ...
    try:
        ...
        with dai.Device(oak_pipeline) as device:
            print(f"Connected to OAK-D camera: {device.getDeviceInfo().getMxId()}")
            ...
            try:
                while True:
                    ...
                    # Process the frame with the lane segmentation model
...
def index():
    ...
    <html>
    <head>
        <title>Lane Segmentation Stream</title>
        <style>
            body {
    ...
    <body>
        <h1>Lane Segmentation Stream</h1>
        <div class="model-info">
            <h3>Active Model:</h3>
            <p><strong>Lane Segmentation:</strong> lane-segmentation-2.0 (v4)</p>
        </div>
