...
# Local Roboflow Inference Server URLs
OBJECT_DETECTION_URL = f"http://localhost:9001/{OBJECT_DETECTION_MODEL['MODEL_ID']}/{OBJECT_DETECTION_MODEL['VERSION']}"
LANE_SEGMENTATION_URL = f"http://localhost:9001/{LANE_SEGMENTATION_MODEL['MODEL_ID']}/{LANE_SEGMENTATION_MODEL['VERSION']}"
...
def process_segmentation(frame, results):
    ...
    try:
        ...
        # Create a color mask (blue for lane markings)
...
def process_object_detection(frame, results):
    """Process object detection results and draw them on frame"""
...
def make_inference_request(url, api_key, frame):
...
def process_frame_with_inference(frame, frame_id):
    """Send frame to Roboflow Inference Server and process results from both models"""
    global latest_frame
    
    # Make object detection request
    object_detection_results = make_inference_request(
        OBJECT_DETECTION_URL, 
        OBJECT_DETECTION_MODEL["API_KEY"], 
        frame
    )
    
    # Make lane segmentation request
    lane_segmentation_results = make_inference_request(
        LANE_SEGMENTATION_URL, 
        LANE_SEGMENTATION_MODEL["API_KEY"], 
        frame
    )
    
    # Process object detection results
    if object_detection_results:
        prediction_count = len(object_detection_results.get('predictions', []))
        print(f"Frame {frame_id} - Object detection predictions: {prediction_count}")
        frame = process_object_detection(frame, object_detection_results)
    
    # Process lane segmentation results
    if lane_segmentation_results:
        print(f"Frame {frame_id} - Lane segmentation processed")
        frame = process_segmentation(frame, lane_segmentation_results)
...
def camera_loop():
    ...
    print("Starting OAK-D Pro with dual model Roboflow Inference...")
    print(f"Object Detection Model: {OBJECT_DETECTION_MODEL['MODEL_ID']}/{OBJECT_DETECTION_MODEL['VERSION']}")
    print(f"Lane Segmentation Model: {LANE_SEGMENTATION_MODEL['MODEL_ID']}/{LANE_SEGMENTATION_MODEL['VERSION']}")
        ...
        with dai.Device(oak_pipeline) as device:
            ...
            try:
                while True:
                    ...
                    # Process the frame with both Roboflow models
...
def index():
    ...
    <html>
    <head>
        <title>OAK-D Pro Dual Model Detection</title>
        <style>
            body {
    ...
    <body>
        <h1>OAK-D Pro Dual Model Detection</h1>
        <div class="model-info">
            <h3>Active Models:</h3>
            <p><strong>Object Detection:</strong> barrels-potholes-markers-test (v5)</p>
            <p><strong>Lane Segmentation:</strong> lane-segmentation-2.0-xgzqs (v1)</p>
        </div>
        <div class="video-container">
            <img src="{{ url_for('video_feed') }}" alt="Object Detection Stream">
        </div>
    </body>
    </html>
    """
